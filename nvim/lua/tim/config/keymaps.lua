...
function M.setup()
	...
	vim.keymap.set("v", "<leader>so", ":sort<CR>", { desc = "Sort lines alphabetically" })
	...
	vim.keymap.set("n", "<leader>wm", function()
		-- Jump to the largest normal window (main editing area)
		...
		for _, win in ipairs(vim.api.nvim_list_wins()) do
			...
			-- Skip special windows (trouble, oil, terminal, etc.)
			if buftype == "" and filetype ~= "oil" and filetype ~= "trouble" then
	...
	end, { desc = "Jump to main window" })

	vim.keymap.set("n", "<leader>wo", function()
		-- Close all windows except current, but keep special windows at bottom
		...
		for _, win in ipairs(vim.api.nvim_list_wins()) do
			if win ~= current_win then
				...
				local filetype = vim.bo[buf].filetype
				local buftype = vim.bo[buf].buftype
				-- Close normal windows, keep Trouble/special windows
				if buftype == "" and filetype ~= "trouble" then
					vim.api.nvim_win_close(win, false)
				end
			end
		end
	end, { desc = "Close all but main window" })
	...
	end, { desc = "[S]earch [B]uffers (Snacks)" })
	vim.keymap.set("n", "<leader>sr", function()
		Snacks.picker.resume()
	end, { desc = "[S]earch [R]esume (Snacks)" })
	vim.keymap.set("n", "<leader>sw", function()
		Snacks.picker.grep_word()
	end, { desc = "[S]earch current [W]ord (Snacks)" })

	-- Git pickers
	vim.keymap.set("n", "<leader>gf", function()
		Snacks.picker.git_files()
	end, { desc = "[G]it [F]iles (Snacks)" })
	vim.keymap.set("n", "<leader>gs", function()
		Snacks.picker.git_status()
	end, { desc = "[G]it [S]tatus (Snacks)" })
	vim.keymap.set("n", "<leader>gc", function()
		Snacks.picker.git_log()
	end, { desc = "[G]it [C]ommits (Snacks)" })

	-- LSP pickers
	vim.keymap.set("n", "<leader>ld", function()
		Snacks.picker.diagnostics()
	end, { desc = "[L]SP [D]iagnostics (Snacks)" })
	vim.keymap.set("n", "<leader>ls", function()
		Snacks.picker.lsp_symbols()
	end, { desc = "[L]SP [S]ymbols (Snacks)" })
	vim.keymap.set("n", "<leader>lr", function()
		Snacks.picker.lsp_references()
	end, { desc = "[L]SP [R]eferences (Snacks)" })

	-- Utility pickers
	vim.keymap.set("n", "<leader>sk", function()
		Snacks.picker.keymaps()
	end, { desc = "[S]earch [K]eymaps (Snacks)" })
	vim.keymap.set("n", "<leader>sH", function()
		Snacks.picker.help()
	end, { desc = "[S]earch [H]elp (Snacks)" })
	vim.keymap.set("n", "<leader>sc", function()
		Snacks.picker.command_history()
	end, { desc = "[S]earch [C]ommand History (Snacks)" })

	-- Quick alternative mappings (can replace telescope eventually)
	vim.keymap.set("n", "<C-p>", function()
		Snacks.picker.files()
	end, { desc = "Quick File Search (Snacks)" })
	vim.keymap.set("n", "<leader>/", function()
		Snacks.picker.grep()
	end, { desc = "Quick Grep Search (Snacks)" })
end
