return {
  ...
  config = function()
    ...
    -- Configure LSP handlers for better hover and signature help
    ...
    -- Get the default config and extend it
    local sourcekit_config = require('lspconfig.configs.sourcekit').default_config
    
    lspconfig.sourcekit.setup(vim.tbl_deep_extend('force', sourcekit_config, {
      capabilities = capabilities,
      ...
      on_attach = function(client, bufnr)
        -- Immediately detach from Package.swift files
        local filename = vim.api.nvim_buf_get_name(bufnr)
        if filename:match("Package%.swift$") then
      ...
      root_dir = function(filename)
        -- Don't start LSP for Package.swift files
        if filename:match("Package%.swift$") then
          return nil
        end
        return util.root_pattern("buildServer.json", "*.xcodeproj", "*.xcworkspace", "compile_commands.json", "Package.swift", ".git")(filename)
      end,
      settings = {
        sourcekit = {
